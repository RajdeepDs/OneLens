...
const buttonVariants = cva(
	"group/button flex shrink-0 select-none items-center justify-center whitespace-nowrap rounded-lg border border-transparent bg-clip-padding font-normal text-[13px] outline-none transition-transform duration-160 ease-out focus-visible:border-ring focus-visible:ring-3 focus-visible:ring-ring/50 active:scale-[0.96] disabled:pointer-events-none disabled:opacity-50 aria-invalid:border-destructive aria-invalid:ring-3 aria-invalid:ring-destructive/20 dark:aria-invalid:border-destructive/50 dark:aria-invalid:ring-destructive/40 [&_svg:not([class*='size-'])]:size-4 [&_svg]:pointer-events-none [&_svg]:shrink-0",
	{
		variants: {
			variant: {
				default:
					"bg-primary text-primary-foreground transition-colors duration-150 ease-[var(--ease-out)] [a]:hover:bg-primary/80",
				outline:
				...
				secondary:
					"inset-shadow bg-white text-secondary-foreground shadow-xs transition-colors duration-150 ease-[var(--ease-out)] hover:bg-gray-2 aria-expanded:bg-white aria-expanded:text-secondary-foreground dark:bg-gray-2",
				ghost:
				...
				destructive:
					"bg-destructive/10 text-destructive transition-colors duration-150 ease-[var(--ease-out)] hover:bg-destructive/20 focus-visible:border-destructive/40 focus-visible:ring-destructive/20 dark:bg-destructive/20 dark:focus-visible:ring-destructive/40 dark:hover:bg-destructive/30",
...
function renderShortcut(shortcut?: Shortcut, isLink = false): ReactNode {
	...
	if (Array.isArray(normalized)) {
		return (
			<KbdGroup
				className="flex items-center gap-1.5 text-gray-1"
				data-shortcut="true"
			>
	...
	return (
		<Kbd
			className={cn(
				"text-gray-10 uppercase",
				!isLink &&
					"pointer-events-none absolute top-1/2 right-2.5 size-5 -translate-y-1/2"
			)}
			data-shortcut="true"
		>
		...
		<MaybeTooltip tooltip={tooltip}>
			...
			>
				{icon &&
					cloneElement(icon, {
						className: cn(
							"pointer-events-none absolute top-1/2 left-2.5 size-5 -translate-y-1/2",
