...
import { Separator } from "@onelens/ui/components/separator";
import {
	Sheet,
	SheetContent,
	SheetDescription,
	SheetHeader,
	SheetTitle,
} from "@onelens/ui/components/sheet";
import { Skeleton } from "@onelens/ui/components/skeleton";
import {
	Tooltip,
	TooltipContent,
	TooltipTrigger,
} from "@onelens/ui/components/tooltip";
...
function SidebarProvider({
	...
	const setOpen = React.useCallback(
		(value: boolean | ((value: boolean) => boolean)) => {
			...
			// biome-ignore lint: This is a standard cookie operation for state persistence
...
function Sidebar({
	...
	if (collapsible === "none") {
		return (
			<div
				className={cn(
					"flex h-full w-(--sidebar-width) flex-col bg-gray-1 text-sidebar-foreground",
					className
				)}
				data-slot="sidebar"
				{...props}
	...
	return (
		...
		>
			...
			>
				<div
					className="flex size-full flex-col bg-gray-3 group-data-[variant=floating]:rounded-lg group-data-[variant=floating]:shadow-sm group-data-[variant=floating]:ring-1 group-data-[variant=floating]:ring-sidebar-border"
					data-sidebar="sidebar"
...
function SidebarGroupContent({
	...
	return (
		<div
			className={cn("w-full text-body-small-spaced", className)}
...
const sidebarMenuButtonVariants = cva(
	"peer/menu-button group/menu-button flex w-full items-center gap-2 overflow-hidden rounded-md p-2 text-left text-body-small-spaced outline-hidden ring-sidebar-ring transition-[width,height,padding] hover:bg-sidebar-accent hover:text-sidebar-accent-foreground focus-visible:ring-2 active:bg-sidebar-accent active:text-sidebar-accent-foreground disabled:pointer-events-none disabled:opacity-50 group-has-data-[sidebar=menu-action]/menu-item:pr-8 aria-disabled:pointer-events-none aria-disabled:opacity-50 data-active:bg-sidebar-accent data-active:font-medium data-active:text-sidebar-accent-foreground data-open:hover:bg-sidebar-accent data-open:hover:text-sidebar-accent-foreground group-data-[collapsible=icon]:size-8! group-data-[collapsible=icon]:p-2! [&>span:last-child]:truncate [&_svg]:size-4 [&_svg]:shrink-0",
	{
		variants: {
			variant: {
			...
			size: {
				default: "h-8 text-body-small-spaced",
				sm: "h-7 text-xs",
				lg: "h-12 text-body-small-spaced group-data-[collapsible=icon]:p-0!",
			},
		},
		defaultVariants: {
			variant: "default",
